...
class cls_currency:
    def __init__(self, label:str, factor:int=1, number_of_days_1year:int=365):
        self.label = label
        self.factor = factor
        self.number_of_days_1year = number_of_days_1year

class cls_currency_pair:
    def __init__(self, base:cls_currency, underlying:cls_currency, quotation_mode:quotation_mode_enum, swap_point_factor:int=10000, number_of_days_1year:int=360, day_shift:int=2):
        self.base = base
        ...
        elif quotation_mode == quotation_mode_enum.udn_base:
    ...
    def get_reversed_quotation_mode(self):
        if self.quotation_mode == quotation_mode_enum.base_und:
            quotation_mode = quotation_mode_enum.udn_base
        elif self.quotation_mode == quotation_mode_enum.udn_base:
    ...
    def get_reversed_pair(self):
        return cls_currency_pair(self.base, self.underlying, self.get_reversed_quotation_mode(), self.swap_point_factor, self.number_of_days_1year, self.day_shift)


class cls_tenor:
    def __init__(self, label:str, start_date:datetime.date, maturity_date:datetime.date):
        self.label = label
    ...
    def get_inverse_tenor(self, label:str):
        return cls_tenor(self.label + " inverse", self.maturity_date,self.start_date)
...
class cls_rate:
    def __init__(self, tenor:cls_tenor, mid:float=0, bid:float=0, ask:float=0):
        self.tenor = tenor
        self.__mid =0
        self.__bid = 0
        self.__ask = 0
...
class cls_single_currency_rate(cls_rate):
    def __init__(self, currency:cls_currency,tenor:cls_tenor, mid:float=0, bid:float=0, ask:float=0):
...
class cls_discount_factor(cls_single_currency_rate):
    ...
    def get_discount_rate(self):
        return cls_discount_rate(self.currency,self.tenor, ( 1/self.mid - 1) * self.currency.number_of_days_1year / self.tenor.number_of_days )
...
class cls_capitalized_factor(cls_single_currency_rate):
    ...
    def get_discount_rate(self):
        return cls_discount_rate(self.currency,self.tenor, ( self.mid - 1) * self.currency.number_of_days_1year / self.tenor.number_of_days )


class cls_market_quote(cls_single_currency_rate):
    def get_discount_factor(self,discount_factor_today_spot:cls_discount_factor):
        ...
        if self.tenor.maturity_date > discount_factor_today_spot.tenor.maturity_date:
            ...
            ds_value = self.currency.number_of_days_1year * self.currency.factor * (1/discount_factor_today_spot.mid * (1+self.mid * number_of_days_spot_to_maturity / self.currency.factor / self.currency.number_of_days_1year) -1)/number_of_days_today_to_maturity

            return cls_discount_rate(self.currency,cls_tenor("",discount_factor_today_spot.tenor.start_date, self.tenor.maturity_date) ,ds_value)


class cls_discount_rate(cls_single_currency_rate):
    def get_discount_factor(self):
        return cls_discount_factor(self.currency, self.tenor, 1/(1+self.mid * self.currency.number_of_days_1year / self.tenor.number_of_days))

    def get_capitalized_factor(self):
        return cls_capitalized_factor(self.currency, self.tenor, 1+self.mid * self.currency.number_of_days_1year / self.tenor.number_of_days)

    def get_market_quote(self,discount_factor_today_spot:cls_discount_factor)->cls_market_quote:
        ...
        if self.tenor.maturity_date > discount_factor_today_spot.tenor.maturity_date:
            ...
            market_quote_value = self.currency.number_of_days_1year * self.currency.factor * (discount_factor_today_spot.mid * (1+self.mid * number_of_days_today_to_maturity / self.currency.factor / self.currency.number_of_days_1year) -1)/number_of_days_spot_to_maturity

            return cls_market_quote(self.currency,cls_tenor("",discount_factor_today_spot.tenor.maturity_date, self.tenor.maturity_date) ,market_quote_value)

class cls_currency_pair_rate(cls_rate):
    def __init__(self, currency_pair:cls_currency_pair, tenor:cls_tenor=None, mid:float=0, bid:float=0, ask:float=0):
...
class cls_fx_rate(cls_currency_pair_rate):
    ...
    def get_reversed_fx_rate(self):
        return cls_fx_rate(self.currency_pair.get_reversed_pair(), self.tenor, 1/self.mid, 1/self.bid, 1/self.ask)
    ...
    def init_by_cross_fx_rate(self,fx_rate_1, fx_rate_2):
        ...
        fx1_base_und = fx_rate_1.get_fx_rate_by_quotation_mode(quotation_mode_enum.base_und)
        fx2_base_und = fx_rate_2.get_fx_rate_by_quotation_mode(quotation_mode_enum.base_und)
        ...
        if self.currency_pair.quotation == build_quotation(fx1_base_und.currency_pair.base.label, fx2_base_und.currency_pair.base.label) \
            ...
            self.set_rate_by_bid_ask(fx1_base_und.bid / fx2_base_und.ask, fx1_base_und.ask / fx2_base_und.bid)
            ...
            self.set_rate_by_bid_ask(fx1_base_und.bid * fx2_base_und.bid, fx1_base_und.ask * fx2_base_und.ask)
            ...
            self.set_rate_by_bid_ask(1/(fx1_base_und.ask * fx2_base_und.ask), 1/(fx1_base_und.bid * fx2_base_und.bid))
            ...
            self.set_rate_by_bid_ask(fx2_base_und.bid / fx1_base_und.ask, fx2_base_und.ask / fx1_base_und.bid)
            ...
            self.set_rate_by_bid_ask(fx1_base_und.ask / fx2_base_und.bid, fx1_base_und.bid / fx2_base_und.ask)

class cls_fx_spot_rate(cls_fx_rate):
    def __init__(self, currency_pair:cls_currency_pair, tenor:cls_tenor, mid:float=0, bid:float=0, ask:float=0):
...
class cls_fx_forward_rate(cls_fx_rate):
    ...
    def spot_rate(self,spot_rate_value:cls_rate):
        self.__spot_rate.set_rate_by_mid_bid_ask(spot_rate_value.mid, spot_rate_value.bid, spot_rate_value.ask)
        self.set_rate_by_mid_bid_ask(spot_rate_value.mid + self.__swap_point.mid / self.currency_pair.swap_point_factor,
                                     spot_rate_value.bid + self.__swap_point.bid / self.currency_pair.swap_point_factor,
                                     spot_rate_value.ask + self.__swap_point.ask / self.currency_pair.swap_point_factor)
    ...
    def swap_point(self,swap_point_value:cls_rate):
        self.set_rate_by_mid_bid_ask(swap_point_value.mid, swap_point_value.bid, swap_point_value.ask)

        self.set_rate_by_mid_bid_ask(self.__spot_rate.mid + swap_point_value.mid / self.currency_pair.swap_point_factor,
    ...
    def set_forward_rate_by_spot_and_df(self,spot_rate:cls_fx_spot_rate,base_ccy_df_s_m:cls_discount_factor,underlying_ccy_df_s_m:cls_discount_factor):
        ...
        if self.currency_pair.quotation_mode == quotation_mode_enum.base_und:
            self.mid = self.currency_pair.swap_point_factor * spot_rate_with_aligned_quotation_mode.mid * (base_ccy_df_s_m.mid/underlying_ccy_df_s_m.mid - 1 )
        elif self.currency_pair.quotation_mode == quotation_mode_enum.udn_base:
            self.mid = self.currency_pair.swap_point_factor * spot_rate_with_aligned_quotation_mode.mid * (underlying_ccy_df_s_m.mid/base_ccy_df_s_m.mid - 1 )

    def set_forward_rate_by_spot_and_swp(self,spot_rate_value:cls_rate,swap_point_value:cls_rate):
...
class cls_swap_point(cls_fx_rate):
    ...
    def set_swap_point_by_setting_forward_rate_and_spot(self,spot_rate:cls_fx_spot_rate,forward_rate:cls_fx_forward_rate):
    ...
    def set_swap_point_by_discount_factor(self,spot_rate:cls_fx_spot_rate,df_base_s_m:cls_discount_factor,df_underlying_s_m:cls_discount_factor):

        if self.tenor.maturity_date > spot_rate.tenor.maturity_date :

            if self.currency_pair.quotation_mode == quotation_mode_enum.base_und :
                self.bid = spot_rate.mid * (df_base_s_m.ask / df_underlying_s_m.bid - 1)
                self.ask = spot_rate.mid * (df_base_s_m.bid / df_underlying_s_m.ask - 1)
            elif self.currency_pair.quotation_mode == quotation_mode_enum.udn_base :
                self.bid = spot_rate.mid * (df_underlying_s_m.ask / df_base_s_m.bid - 1)
                self.ask = spot_rate.mid * (df_underlying_s_m.bid / df_base_s_m.ask - 1)
        else:
            pass
...
class cls_fx_rate_curve():
    def __init__(self,fx_rate_list:list):
        fx_rate_list.sort(key=lambda fx_rate:fx_rate.tenor.start_date,reverse=False)
        fx_rate_list.sort(key=lambda fx_rate:fx_rate.tenor.maturity_date,reverse=False)

        self.fx_rate_list = fx_rate_list
        #debug
        for a in self.fx_rate_list: print(a.tenor.label)

class cls_discount_factor_curve(cls_fx_rate_curve):

    def __init__(self,fx_rate_list:list,linearization:linearization_enum):
        super().__init__(fx_rate_list)
        self.linearization = linearization

    def get_discount_factor_by_interpolation(self, df_early:cls_discount_factor,df_late:cls_discount_factor,tenor_mid:cls_tenor,linearization:linearization_enum) -> cls_discount_factor:
        result=0
        if linearization == linearization_enum.log_linear:
            result = 10^((math.log10(df_late.mid)-math.log10(df_early.mid))/(df_late.tenor.number_of_days-df_early.tenor.number_of_days)*(tenor_mid.number_of_days-df_early.tenor.number_of_days)+math.log10(df_early.mid)   )
        elif linearization == linearization_enum.line_linear:
            result = (df_late.mid-df_early.mid)/(df_late.tenor.number_of_days-df_early.tenor.number_of_days)*(tenor_mid.number_of_days-df_early.tenor.number_of_days)+df_early.mid

        return cls_discount_factor(df_early.currency, tenor_mid, result)


    def get_discount_factor_by_maturity_date(self,maturity_date:datetime.date) -> cls_discount_factor:
        ...
        for iter_discount_factor in self.fx_rate_list:
            ...
            if iter_discount_factor.tenor.maturity_date > maturity_date:
                ...
                print("df_late's maturity date is " + df_late.tenor.maturity_date.strftime('%Y-%m-%d'))

                df_early = previous_df
                print("df_early's maturity date is " + df_early.tenor.maturity_date.strftime('%Y-%m-%d'))
...
class cls_capitalized_factor_curve(cls_fx_rate_curve):
    def __init__(self,fx_rate_list:list,linearization:linearization_enum):
        ...
        super().__init__(fx_rate_list)
        self.linearization = linearization
        self.discount_factor_curve = cls_discount_factor_curve([iter_capitalized_factor.get_discount_factor() for iter_capitalized_factor in self.fx_rate_list],linearization)

    def get_capitalized_factor_by_maturity_date(self,maturity_date:datetime.date) -> cls_capitalized_factor:
        return self.discount_factor_curve.get_discount_factor_by_maturity_date(maturity_date).get_capitalized_factor()
